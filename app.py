...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Warts vs Moles Classifier",
    page_icon="🔬",
    layout="centered",
)

# ── Constants ─────────────────────────────────────────────────────────────────
MODEL_PATH       = Path(__file__).parent / "model" / "skin_warts_moles_model.keras"
CLASS_STATS_PATH = Path(__file__).parent / "model" / "class_stats.npz"
# Alphabetical: Moles (index 0), Warts (index 1) — raw_prob = P(Warts)
CLASS_NAMES      = ["Moles", "Warts"]
IMAGE_SIZE       = (224, 224)
THRESHOLD        = 0.5
...
def predict(model, pil_image: Image.Image):
    """raw_prob = P(Warts) — Warts is CLASS_NAMES[1] (alphabetically second)."""
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.title("🔬 Warts vs Moles Classifier")
st.caption(
    "Upload a skin lesion image and the model will classify it as a "
    "**Wart** or a **Mole** using a MobileNetV2 transfer-learning model."
)
st.divider()
...
uploaded = st.file_uploader(
    ...
    type=["jpg", "jpeg", "png"],
    help="Clear, close-up images of the lesion produce the best results.",
)
...
if uploaded is not None:
    ...
    with col2:
        ...
        if rejected:
            ...
            color = "#2ecc71" if label == "Moles" else "#e74c3c"
            st.markdown(
                f"""
            ...
            with st.expander("Raw probabilities"):
                warts_prob = raw_prob
                moles_prob = 1.0 - raw_prob
                st.write(f"**Warts:** {warts_prob * 100:.2f}%")
                st.progress(warts_prob)
                st.write(f"**Moles:** {moles_prob * 100:.2f}%")
                st.progress(moles_prob)
    ...
    st.info("👆 Upload an image above to get a prediction.", icon="🖼️")
